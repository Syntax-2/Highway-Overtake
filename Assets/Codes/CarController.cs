...
public class CarController : MonoBehaviour
{
    ...
    // --- MODIFIED: Renamed and increased default for better responsiveness ---
    [Tooltip("How quickly torque decreases when NOT accelerating. Higher values make the car stop accelerating faster.")]
    public float decelerationRate = 10000f;
    ...
    void Start()
    {
        ...
        // --- FIX for Jittering ---
        // Set the Rigidbody's interpolation to smooth out visual movement.
    ...
    void FixedUpdate()
    {
        ...
        // --- MODIFIED: Acceleration / Deceleration Logic ---
        if (_gasPressed && !_brakePressed)
        {
            // Ramp up torque when gas is pressed
            _currentAppliedTorque = Mathf.MoveTowards(_currentAppliedTorque, maxMotorTorque, accelerationRate * Time.fixedDeltaTime);
        }
        else
        {
            // Ramp down torque much faster when gas is NOT pressed
            _currentAppliedTorque = Mathf.MoveTowards(_currentAppliedTorque, 0, decelerationRate * Time.fixedDeltaTime);
        }
        // --- END MODIFICATION ---
        ...
        float motorForce = ForwardDirection ? speedLimitedTorque : -speedLimitedTorque;
    ...
    {
        if (_isSpeedBoostActive && _speedBoostCoroutine != null) StopCoroutine(_speedBoostCoroutine);
    ...
    private void InitializeBoostEffects()
    {
        ...
        if (mainCam != null)
        {
            ...
            main.startLifetime = 0.5f; main.startSpeed = -50f; main.startSize = new ParticleSystem.MinMaxCurve(0.01f, 0.05f);
            main.maxParticles = 200; main.loop = true; main.playOnAwake = false;
            var emission = _speedLines.emission; emission.rateOverTime = 150;
            var shape = _speedLines.shape; shape.shapeType = ParticleSystemShapeType.Cone; shape.angle = 25; shape.radius = 5;
            var renderer = _speedLines.GetComponent<ParticleSystemRenderer>(); renderer.renderMode = ParticleSystemRenderMode.Stretch; renderer.lengthScale = 2.5f;
            _speedLines.Stop();
        }
        ...
        if (boostSound != null) { _boostAudioSource.clip = boostSound; }
        ...
        if (canvas != null)
        {
            ...
            rect.pivot = new Vector2(0.5f, 0); rect.anchoredPosition = new Vector2(0, 50);
